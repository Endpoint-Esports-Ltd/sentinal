...
export function registerRegisterPlanCommand(program: Command): void {
  program
    ...
    .action(
      (
        planPath: string,
        opts: { session?: string; project: string; json?: boolean },
      ) => {
        ...
        if (!existsSync(absPath)) {
          if (opts.json) {
            console.log(
              JSON.stringify({ error: `File not found: ${absPath}` }),
            );
          } else {
        ...
        try {
          ...
          if (opts.json) {
            ...
            console.log(
              `Registered: ${spec.id} (${spec.status}, ${done}/${spec.tasks.length} tasks)`,
            );
            if (spec.metadata.worktree) {
              console.log(
                `  Worktree: Yes — use \`sentinal worktree create\` to create an isolated worktree`,
              );
            }
          }
        } catch (err) {
