 ...
 * Generated at: 2026-03-16T11:22:04.184Z
 *
...
var IMPL_EXTENSIONS = [".ts", ".tsx", ".js", ".jsx", ".go", ".py", ".rs", ".c", ".cpp"];
...
var CreateObservationSchema = ObservationSchema.omit({ id: true, qualityScore: true });
...
import { existsSync as existsSync5, readFileSync as readFileSync4, writeFileSync as writeFileSync3, appendFileSync, mkdirSync as mkdirSync2, unlinkSync } from "node:fs";
  ...
  return {
    "tool.execute.before": async (input, output) => {
      ...
      if (tool === "grep" && typeof args.pattern === "string") {
        ...
        if (grepHint)
          await client.app.log({ body: { service: "sentinal", level: "info", message: grepHint } });
      }
      if (tool === "fetch") {
      ...
      if (sidecar && typeof filePath === "string") {
        ...
        if (guide)
          await client.app.log({ body: { service: "sentinal", level: "info", message: guide } });
      }
    },
    ...
    event: async ({ event }) => {
      if (event.type === "session.created") {
        ...
        if (sidecar) {
          ...
          if (conflictMsg)
            await client.app.log({ body: { service: "sentinal", level: "warn", message: conflictMsg } });
        }
