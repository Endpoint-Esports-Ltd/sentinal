 ...
 * Generated at: 2026-03-14T22:57:33.467Z
 *
...
import { existsSync as existsSync4, readFileSync as readFileSync3, writeFileSync as writeFileSync2, appendFileSync, mkdirSync, unlinkSync } from "node:fs";
...
var SENTINAL_DIR = join5(homedir2(), ".sentinal");
var DEBUG_LOG_PATH = join5(SENTINAL_DIR, "plugin.debug.log");
...
function autoStartProcess(pidFile, ...args) {
  const pidPath = join5(SENTINAL_DIR, pidFile);
  ...
  const binPath = join5(SENTINAL_DIR, "bin", "sentinal");
...
var SentinalPlugin = async ({ project, client, $, directory, worktree }) => {
  ...
  const memoryEnabled = (() => {
    try {
      const cfgPath = join5(SENTINAL_DIR, "config.json");
...
\`).slice(0, 500);
    ...
    "experimental.session.compacting": async (input, output) => {
      ...
      const stateDir = join5(projectRoot, ".sentinal");
      ...
      writeFileSync2(join5(stateDir, "compact-state.json"), JSON.stringify(state, null, 2));
      if (activePlan) {
...
Use \\\`/spec \${activePlan}\\\` to resume the workflow.\`);
    ...
    event: async ({ event }) => {
      if (event.type === "session.created") {
        ...
        const stateFile = join5(projectRoot, ".sentinal", "compact-state.json");
...
Resume with: /spec \${state.activePlan}\` } });
      ...
      if (event.type === "session.deleted") {
        ...
        const bufferPath = join5(projectRoot, ".sentinal", "event-buffer.json");
        try {
