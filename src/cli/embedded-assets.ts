 ...
 * Generated at: 2026-06-10T12:23:51.076Z
 *
...
var init_types = __esm(() => {
  ...
  ObservationSchema = z2.object({
    ...
    qualityScore: z2.number().min(0).max(1).default(1)
  });
  CreateObservationSchema = ObservationSchema.omit({
    id: true,
    qualityScore: true
  });
  SessionSchema = z2.object({
    id: z2.string(),
    startTime: z2.number(),
...
var argsSchema = {
  file_path: z3.string().optional().describe("Absolute path to implementation file. If omitted, lists all active TDD states."),
  spec_id: z3.string().optional().describe("Filter by spec ID (only used when file_path is omitted).")
};
    ...
    async remove(config) {
      try {
        ...
        if (!planPath)
          return;
        const slug = slugFromPlanPath(planPath);
        const wt = await sidecar.resolveWorktreeBySlug(slug, config.directory ?? "");
        if (!wt)
          return;
