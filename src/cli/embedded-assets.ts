 ...
 * Generated at: 2026-04-21T10:20:01.281Z
 *
...
// targets/opencode/plugins/sentinal-helpers.js
...
function resolveProjectRoot(worktree, directory, opts) {
  ...
  const checkExists = opts?.exists ?? existsSync5;
...
\`).length;
          ...
          if (!isTestFile(filePath)) {
            ...
            if (testPaths.length > 0 && !testPaths.some((tp) => existsSync6(tp))) {
...
\`));
    ...
    event: async ({ event }) => {
      if (event.type === "session.created") {
        ...
        const stateFile = projectRoot ? join6(projectRoot, ".sentinal", "compact-state.json") : null;
        if (stateFile && existsSync6(stateFile)) {
          try {
            const state = JSON.parse(readFileSync4(stateFile, "utf-8"));
            if (state.activePlan && existsSync6(state.activePlan)) {
              await client.app.log({
                body: {
                  service: "sentinal",
                  level: "info",
