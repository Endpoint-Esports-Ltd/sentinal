...
function makeTmpDb(): string {
  const dir = join(tmpdir(), `sentinal-cli-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe("runSearch", () => {
  ...
  it("should show usage when no query", async () => {
    const result = await runSearch(service, { command: "search", positional: [], flags: {} });
    expect(result).toContain("Usage:");
  });

  it("should find matching observations", async () => {
    service.addObservation(makeObservation({ title: "Database migration" }));
    const result = await runSearch(service, { command: "search", positional: ["migration"], flags: {} });
    expect(result).toContain("Database migration");
  });

  it("should report no matches", async () => {
    const result = await runSearch(service, { command: "search", positional: ["nonexistent"], flags: {} });
...
describe("runList", () => {
  ...
  it("should list recent observations", async () => {
    ...
    const result = await runList(service, { command: "list", positional: [], flags: {} });
  ...
  it("should filter by type", async () => {
    ...
    const result = await runList(service, { command: "list", positional: [], flags: { type: "decision" } });
...
describe("runTimeline", () => {
  ...
  it("should show usage when no anchor", () => {
    const result = runTimeline(service, { command: "timeline", positional: [], flags: {} });
    expect(result).toContain("Usage:");
  });
  ...
  it("should show timeline around anchor", () => {
    ...
    const result = runTimeline(service, { command: "timeline", positional: [], flags: { anchor: String(ids[2]) } });
  ...
  it("should accept anchor as positional arg", () => {
    ...
    const result = runTimeline(service, { command: "timeline", positional: [String(o.id)], flags: {} });
...
describe("runGet", () => {
  ...
  it("should show usage when no IDs", () => {
    const result = runGet(service, { command: "get", positional: [], flags: {} });
    expect(result).toContain("Usage:");
  });
  ...
  it("should show full observation", () => {
    ...
    const result = runGet(service, { command: "get", positional: [String(o.id)], flags: {} });
  ...
  it("should handle multiple IDs", () => {
    ...
    const result = runGet(service, { command: "get", positional: [String(o1.id), String(o2.id)], flags: {} });
...
describe("runExport", () => {
  ...
  it("should export as JSON", () => {
    ...
    const result = runExport(service, { command: "export", positional: [], flags: {} });
  ...
  it("should export as markdown", () => {
    ...
    const result = runExport(service, { command: "export", positional: [], flags: { format: "markdown" } });
    expect(result).toContain("## MD export");
  });

  it("should export filtered by project", () => {
    service.addObservation(makeObservation({ projectPath: "/proj/a", title: "A obs" }));
    service.addObservation(makeObservation({ projectPath: "/proj/b", title: "B obs" }));
    const result = runExport(service, { command: "export", positional: [], flags: { project: "/proj/a" } });
    const parsed = JSON.parse(result);
...
describe("runPrune", () => {
  ...
  it("should report invalid duration", () => {
    const result = runPrune(service, { command: "prune", positional: [], flags: { "older-than": "abc" } });
  ...
  it("should prune old observations", () => {
    ...
    service.addObservation(makeObservation({ timestamp: oldTime, title: "Old obs" }));
    service.addObservation(makeObservation({ title: "Recent obs" }));

    const result = runPrune(service, { command: "prune", positional: [], flags: { "older-than": "90d" } });
