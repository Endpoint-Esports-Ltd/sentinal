...
import { isTestFile, getExpectedTestPaths, isGuardedFile, getImplPathForTest, } from "../../../src/utils/tdd.js";
...
import { getGrepHint, getFetchHint, getPreEditGuide, checkSessionConflict, transitionTddState, resolveProjectRoot, } from "./sentinal-helpers.js";
...
// ─── Plugin ──────────────────────────────────────────────────────────────────
export const SentinalPlugin = async ({ project, client, $, directory, worktree, }) => {
    ...
    return {
        ...
        "tool.execute.after": async (input, output) => {
            ...
            if (QUALITY_TOOLS.includes(input.tool) &&
                filePath &&
        ...
        "experimental.chat.system.transform": async (_input, output) => {
            try {
                ...
                if (!Array.isArray(systemArr)) {
                    log(`system.transform: no usable array in output — skipping injection`);
                    return;
                }
                ...
                if (active) {
                    ...
                    const current = spec.tasks.find((t) => t.status === "in-progress")?.title ??
                    ...
                    if (sidecar) {
                        ...
                        if (restored.hasMemory && restored.markdown) {
                            ...
                            if (projectRoot) {
                                ...
                                writeFileSync(join(stDir, "compact-state.json"), JSON.stringify(compactState, null, 2));
                            }
                        }
                    }
                }
...
//# sourceMappingURL=sentinal.js.map