...
**Quality checks (tsc, eslint, prettier) do NOT run automatically on every edit.** Sentinal only performs instant structural checks (file length, companion tests) per edit. You MUST call `quality_report` with `file:` set to each file after finishing edits to it — that is the only mode that auto-fixes (ESLint `--fix` and Prettier `--write`, applied to that one file). Project-wide (no `file`) is **report-only**: it lists unformatted files and ESLint error/warning counts and never rewrites anything.

| Tool                | When                                                                                                                                                                  |
| ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `quality_report`    | **MUST call with `file:` after finishing edits to each file.** Runs tsc + eslint + prettier in one call; auto-fixes only that file. Without `file` it is report-only. |
