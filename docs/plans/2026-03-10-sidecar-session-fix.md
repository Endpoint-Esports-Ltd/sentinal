...
- `src/memory/store.ts` is already 663 lines (over 600 limit). The sidecar should *reduce* code here by moving session/TDD CRUD into sidecar endpoints, not add to it.
...
export class SidecarClient {
  ...
  // TDD State
  async getTddState(filePath: string, projectPath: string): Promise<TddGuardResult>
...
export async function main(): Promise<void> {
  if (!isMemoryEnabled()) { process.exit(0); }
