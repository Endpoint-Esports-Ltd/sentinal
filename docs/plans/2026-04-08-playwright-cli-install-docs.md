...
- **History:** The `playwright-cli` rule was introduced on 2026-03-11 in commit `999cc5f`. No install hint was included at authoring time. This has been a latent gap since day 1 — it's not a regression, just a bug that only surfaces on machines without `@playwright/cli` pre-installed.
